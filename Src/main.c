...
void LedPatternOnBoard(uint8_t Pattern, uint32_t Delay_ms)
{
// bit0 -> LED1, bit1 -> LED2, bit2 -> LED3, bit3 -> LED4
 HAL_GPIO_WritePin(GPIOC, LED_1_Pin, (Pattern & 0x01) ? GPIO_PIN_SET : GPIO_PIN_RESET);
 HAL_GPIO_WritePin(GPIOC, LED_2_Pin, (Pattern & 0x02) ? GPIO_PIN_SET : GPIO_PIN_RESET);
 HAL_GPIO_WritePin(GPIOC, LED_3_Pin, (Pattern & 0x04) ? GPIO_PIN_SET : GPIO_PIN_RESET);
 HAL_GPIO_WritePin(GPIOD, LED_4_Pin, (Pattern & 0x08) ? GPIO_PIN_SET : GPIO_PIN_RESET);
...
void StartMonitorTask(void *argument)
{
  ...
  for(;;)
  {
		LedPatternOnBoard(Pattern, 200);  // delay 200ms
		Pattern <<= 1;
